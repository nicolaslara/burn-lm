...
pub struct AppConfig {
    ...
    /// The prompt tokens one round may prefill in total (`prefill_token_budget`), shared across
    /// every prompt waiting to be admitted. `0` is unbounded.
...
impl App {
    ...
    pub fn new_with_config(host: IpAddr, port: u16, config: AppConfig) -> Self {
        ...
        set_if_unset("BURN_LM_PREFILL_TOKEN_BUDGET", config.prefill_token_budget);
