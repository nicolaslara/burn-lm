...
> [!NOTE]
> **This is a fork.** Upstream [tracel-ai/burn-lm](https://github.com/tracel-ai/burn-lm) was
> archived in September 2026; this fork continues from its final commit and adds:
>
> - **Continuous request batching** — one fused forward advances every in-flight sequence per
>   round, instead of serving one request at a time.
> - **Chunked prefill** — a long prompt is sliced across rounds so it cannot stall the streams
>   already in flight.
> - **Paged KV cache** — the cache lives in 128-token blocks with per-lane block tables, and
>   admission grows a sequence block by block, so concurrency is no longer capped by reserving a
>   full context window per lane.
>
> It also tracks current Burn `main` rather than a pinned revision.
...
```sh
git clone https://github.com/nicolaslara/burn-lm.git
cd burn-lm
cargo burn-lm
```
